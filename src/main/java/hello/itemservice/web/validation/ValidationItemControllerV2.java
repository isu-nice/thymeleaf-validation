...
public class ValidationItemControllerV2 {
    ...
    //    @PostMapping("/add")
    public String addItemV1(@ModelAttribute Item item, BindingResult bindingResult, // [순서 중요!!] 꼭 @ModelAttribute 다음에 BindingResult가 와야 함
        ...
        if (item.getPrice() != null && item.getQuantity() != null) {
            ...
            if (resultPrice < 10_000) {
                bindingResult.addError(new ObjectError("item", "[가격 x 수량]은 10,000원 이상이어야 합니다. 현재 가격 = " + resultPrice));
            }
        }
    ...
    public String addItemV2(@ModelAttribute Item item, BindingResult bindingResult, RedirectAttributes redirectAttributes) {
        ...
        if (item.getPrice() != null && item.getQuantity() != null) {
            int resultPrice = item.getPrice() * item.getQuantity();
            if (resultPrice < 10_000) {
                bindingResult.addError(new ObjectError("item", null, null, "[가격 x 수량]은 10,000원 이상이어야 합니다. 현재 가격 = " + resultPrice));
            }
        }
